...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error in PATCH /api/mileage-logs/[id]:", error instanceof Error ? error.message : String(error));
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Error in DELETE /api/mileage-logs/[id]:", error instanceof Error ? error.message : String(error));
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
