...
export function PaystubsExportDialog({
  open,
  onOpenChange,
  paystubs,
}: PaystubsExportDialogProps) {
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [exporting, setExporting] = useState(false);

  // Initialize with all paystubs selected
  useEffect(() => {
    if (open && paystubs.length > 0) {
      setSelectedIds(new Set(paystubs.map((p) => p.id)));
    }
  }, [open, paystubs]);

  const handleToggleAll = () => {
    if (selectedIds.size === paystubs.length) {
  ...
  const handleExport = async (exportAll: boolean) => {
    ...
    try {
      ...
      const response = await fetch("/api/paystubs/export-pdf", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          paystubIds: idsToExport,
        }),
      });
      ...
      a.href = url;
      a.download = `paystubs-${new Date().toISOString().split("T")[0]}.pdf`;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Export Paystubs to PDF</DialogTitle>
          <DialogDescription>
            Select which paystubs to include in the PDF export
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={exporting || paystubs.length === 0}
          >
            {exporting ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
